...
The Monarch environment must include `$MONARCH_REALM_ROOT/.monarch/bin` and
`$HOME/.local/bin` on `PATH` so realm-local helpers, user-local tools, and
container-installed shortcuts remain available in interactive and Nix
development shells.
